...
le_void_t er_voxel_display_cube(er_voxel_t const * const voxel, const le_real_t er_lon, const le_real_t er_lat)
{
    ...
    for (int o = -1; o <= 1; o += 2) {
        ...
        point[1] = voxel->vx_edge[1] + 0.5 * o * (slon * slat * s[0] + clat * s[1] - clon * slat * s[2]);
        point[2] = voxel->vx_edge[2] + 0.5 * o * (-slon * clat * s[0] + slat * s[1] + clon * clat * s[2]);
        
        point[21] = point[3] = point[0] - o * clon * s[0];
        point[22] = point[4] = point[1] - o * slon * slat * s[0];
        point[23] = point[5] = point[2] + o * slon * clat * s[0];
        
        point[6] = point[3] - o * slon * s[2];
        point[7] = point[4] + o * clon * slat * s[2];
        point[8] = point[5] - o * clon * clat * s[2];
        
        point[9] = point[6] + o * clon * s[0];
        point[10] = point[7] + o * slon * slat * s[0];
        point[11] = point[8] - o * slon * clat * s[0];
        
        point[12] = point[9];
        point[13] = point[10] - o * clat * s[1];
        point[14] = point[11] - o * slat * s[1];
        
        point[15] = point[12] + o * slon * s[2];
        point[16] = point[13] - o * clon * slat * s[2];
        point[17] = point[14] + o * clon * clat * s[2];
        
        point[18] = point[15] - o * clon * s[0];
        point[19] = point[16] - o * slon * slat * s[0];
        point[20] = point[17] + o * slon * clat * s[0];
